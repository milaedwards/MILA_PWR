...
class Config:
    ...
    RHO_NOM: float = 726.51  # nominal primary density [kg/m^3] (@15.5 MPa, 300°C)
    CP_PRI: float = 5458.0  # primary heat capacity [J/(kg*K)]
    M_DOT_PRI = 14520.72  # [kg/s]
    V_PRIMARY: float = 271.84 # [m^3]
    ...
    T_SAT_SEC: float = 546.13  # Saturation temp at P_sec [K] (523.37 °F)
    M_DOT_SEC: float = 1887.4 # kg/s steam mass flow rate at full power (1.498e7 lbm/hr)

    # Starting conditions (t=0)
    T_HOT_INIT: float = 595.59  # Hot-leg initial temperature [K] (612.2 °F)
    T_COLD_INIT: float = 552.59  # Cold-leg initial temperature [K] (535.0 °F)
    P_PRI_INIT: float = P_PRI_SET  # Initial primary pressure [Pa]
    P_SEC_INIT: float = P_SEC_CONST  # Initial secondary pressure [Pa]
